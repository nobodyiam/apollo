...
function directive($window, $translate, toastr, AppUtil, EventManager, PermissionService,
    NamespaceLockService,
    UserService, CommitService, ReleaseService, InstanceService, NamespaceBranchService,
    ConfigService) {
  return {
    restrict: 'E',
    ...
    link: function (scope) {
      ...
      function initNamespace(namespace, viewType) {
        ...
        function initLinkedNamespace(namespace) {
          if (!namespace.isPublic || !namespace.isLinkedNamespace || namespace.format
              != 'properties') {
            return;
          }
          //load public namespace
          ConfigService.load_public_namespace_for_associated_namespace(scope.env, scope.appId,
              scope.cluster,
              namespace.baseInfo.namespaceName)
          .then(function (result) {
      ...
      function loadCommitHistory(namespace) {
        ...
        .then(function (result) {
          ...
          toastr.error(AppUtil.errorMsg(result),
              $translate.instant('ApolloNsPanel.LoadingHistoryError'));
        });
      }
      ...
      function initRules(branch) {
        ...
        .then(function (result) {
          ...
          toastr.error(AppUtil.errorMsg(result),
              $translate.instant('ApolloNsPanel.LoadingGrayscaleError'));
        });
      ...
      function modifyByText(namespace) {
        ...
        ConfigService.modify_items(scope.appId,
            ...
            function (result) {
              ...
              toastr.error(AppUtil.errorMsg(result),
                  $translate.instant('ApolloNsPanel.ModifyFailed'));
              namespace.commitChangeBtnDisabled = false;
              return false;
            }
        );
      ...
      scope.aceConfig = {
        ...
        theme: 'eclipse',
        mode: scope.namespace.format === 'yml' ? 'yaml' : (scope.namespace.format === 'txt'
            ? undefined : scope.namespace.format),
