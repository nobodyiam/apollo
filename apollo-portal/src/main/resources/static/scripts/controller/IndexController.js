index_module.controller('IndexController',
    ['$scope', '$window', '$translate', 'toastr', 'AppUtil', 'AppService',
      'UserService', 'FavoriteService',
      IndexController]);

function IndexController($scope, $window, $translate, toastr, AppUtil, AppService, UserService,
    FavoriteService) {
    ...
    AppService.has_create_application_role($scope.userId).then(
        ...
        function (reason) {
          toastr.warning(AppUtil.errorMsg(reason),
              $translate.instant('Index.GetCreateAppRoleFailed'));
        }
    )
  }
