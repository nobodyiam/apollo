application_module.controller("ConfigBaseInfoController",
    ['$rootScope', '$scope', '$window', '$location', '$translate', 'toastr', 'EventManager',
      'UserService',
      'AppService',
      'FavoriteService',
      'PermissionService',
...
function ConfigBaseInfoController($rootScope, $scope, $window, $location, $translate, toastr,
    EventManager, UserService, AppService,
    FavoriteService,
    PermissionService,
    AppUtil) {
    ...
    $scope.missEnvs.forEach(function (env) {
      AppService.create_remote(env, $scope.appBaseInfo).then(function (result) {
        ...
        toastr.error(AppUtil.errorMsg(result),
            `${$translate.instant('Common.CreateFailed')}:${env}`);
        count++;
        if (count == $scope.missEnvs.length) {
          location.reload(true);
        }
