    ...
    ConfigService.load_all_namespaces($rootScope.pageContext.appId,
        ...
        function (result) {
          ...
          toastr.error(AppUtil.errorMsg(result),
              $translate.instant('Config.LoadingAllNamespaceError'));
        });
  }
  ...
  EventManager.subscribe(EventManager.EventType.DELETE_NAMESPACE_FAILED, function (context) {
    ...
    if (context.reason == 'master_instance') {
      ...
      otherAppAssociatedNamespaces.forEach(function (namespace) {
        ...
        var url = AppUtil.prefixPath() + '/config.html?#/appid=' + appId + '&env='
            + $scope.pageContext.env + '&cluster='
            + clusterName;

        namespaceTips.push(
            "<a target='_blank' href=\'" + url + "\'>AppId = " + appId + ", Cluster = "
            + clusterName
            + ", Namespace = " + namespace.namespaceName + "</a>");
      });

      $scope.deleteNamespaceContext.detailReason = $translate.instant(
          'Config.DeleteNamespaceFailedTips') + "<br>" + namespaceTips.join("<br>");
