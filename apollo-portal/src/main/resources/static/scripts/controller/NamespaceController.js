namespace_module.controller("LinkNamespaceController",
    ['$scope', '$location', '$window', '$translate', 'toastr', 'AppService', 'AppUtil',
      'NamespaceService',
      'PermissionService', 'CommonService',
      function ($scope, $location, $window, $translate, toastr, AppService, AppUtil,
          NamespaceService,
          PermissionService, CommonService) {
        ...
        NamespaceService.find_public_namespaces().then(function (result) {
          ...
          toastr.error(AppUtil.errorMsg(result),
              $translate.instant('Namespace.LoadingPublicNamespaceError'));
        });
        ...
        AppService.load($scope.appId).then(function (result) {
          ...
          toastr.error(AppUtil.errorMsg(result),
              $translate.instant('Namespace.LoadingAppInfoError'));
        });
        ...
        $scope.createNamespace = function () {
          if ($scope.type == 'link') {
            ...
            NamespaceService.createAppNamespace($scope.appId, $scope.appNamespace,
                appendNamespacePrefix).then(
                function (result) {
                  $scope.step = 2;
                  setTimeout(function () {
                    $scope.submitBtnDisabled = false;
