cluster_module.controller('ClusterController',
    ['$scope', '$location', '$window', '$translate', 'toastr', 'AppService', 'EnvService',
      'ClusterService',
      'AppUtil',
      function ($scope, $location, $window, $translate, toastr, AppService, EnvService,
          ClusterService,
          AppUtil) {
          ...
          $scope.envs.forEach(function (env) {
            if (env.checked) {
              ...
              ClusterService.create_cluster($scope.appId, env.name,
                ...
                toastr.error(AppUtil.errorMsg(result),
                    $translate.instant('Cluster.ClusterCreateFailed'));
                $scope.submitBtnDisabled = false;
              })
            }
          });
