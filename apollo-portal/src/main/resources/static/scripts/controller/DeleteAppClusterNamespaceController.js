delete_app_cluster_namespace_module.controller('DeleteAppClusterNamespaceController',
    ['$scope', '$translate', 'toastr', 'AppUtil', 'AppService', 'ClusterService',
      'NamespaceService', 'PermissionService',
      DeleteAppClusterNamespaceController]);

function DeleteAppClusterNamespaceController($scope, $translate, toastr, AppUtil, AppService,
    ClusterService, NamespaceService, PermissionService) {
  ...
  function getClusterInfo() {
    ...
    ClusterService.load_cluster($scope.cluster.appId, $scope.cluster.env, $scope.cluster.name).then(
        function (result) {
  ...
  function deleteCluster() {
    ...
    if (confirm(confirmTip)) {
      ClusterService.delete_cluster($scope.cluster.appId, $scope.cluster.env,
          $scope.cluster.name).then(function (result) {
        toastr.success($translate.instant('Delete.Deleted'));
        $scope.deleteClusterBtnDisabled = true;
      }, function (result) {
        AppUtil.showErrorMsg(result);
  ...
  function getAppNamespaceInfo() {
    ...
    NamespaceService.loadAppNamespace($scope.appNamespace.appId, $scope.appNamespace.name).then(
        function (result) {
  ...
  function deleteAppNamespace() {
    ...
    if (confirm(confirmTip)) {
      NamespaceService.deleteAppNamespace($scope.appNamespace.appId, $scope.appNamespace.name).then(
          function (result) {
            toastr.success($translate.instant('Delete.Deleted'));
            $scope.deleteAppNamespaceBtnDisabled = true;
          }, function (result) {
