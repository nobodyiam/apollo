...
function SettingController($scope, $location, $translate, toastr,
  ...
  function initPermission() {
    ...
    .then(function (result) {
      ...
      PermissionService.has_open_manage_app_master_role_limit().then(function (value) {
        ...
        PermissionService.has_manage_app_master_permission($scope.pageContext.appId).then(
            function (res) {
              $scope.hasManageAppMasterPermission = res.hasPermission
                  && $scope.hasAssignUserPermission;

              PermissionService.has_root_permission().then(function (value) {
                $scope.hasManageAppMasterPermission = value.hasPermission
                    || $scope.hasManageAppMasterPermission;
              });
            });
      });
    });
  ...
  function initAppForm(app) {
    ...
    var defaultSelectedDOM = '<option value="' + app.ownerName + '" selected="selected">'
        + app.ownerName
        + '</option>';
